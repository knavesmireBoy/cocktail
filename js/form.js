...
(function() {
	...
	function post(e) {
		...
		_.compose(ptL(setAttrs, harrington_book), anCr(anchor('aside')))('img');
		_.each(bumf, ptL(tagFactory, 'p', _.compose(doText, anchor)));
		klasAdd('response', utils.getBody);
		_.compose(utils.setText('Bartender!, get me a <span>' + cocktail + '!</span>'), anchor)('h3');
		anchor = anchor('section');
		div = anCr(anchor)('div');
		anCr(div)(tbl(data));
		_.compose(ptL(setAttrs, comb_bound), anCr(anCr(anchor)('a')))('img');
		div = _.compose(utils.setText(esquire), twice(invoke)('p'), anCr, ptL(utils.climbDom, 3), ptL(setAttrs, esquire_book), twice(invoke)('img'), anCr, ptL(setAttrs, esquire_link), twice(invoke)('a'), anCr, ptL(klasAdd, 'esquire'), anCr($('response')))('div');
	}
