...
Booze.Iterator = function (rev) {
	"use strict";
	return function (index, coll, validate, doAdvance) {
		var loop = function (bool) {
				if (!bool) {
					index = Booze.Util.doWhen(validate, _.partial(doAdvance, index += 1));
				}
			...
			getnext = function (isRev, bool) {
				...
				Booze.Util.doWhen(isRev(), switchDirection);
			...
			invoke = function (bool) {
				//console.log('invoke..')
				return Booze.Util.getBest(isReversed, [_.bind(back, null, bool), _.bind(forward, null, bool)])();
			},
			ret = {
				forward: forward,
				back: back,
...
Booze.Composite = (function () {
	...
	return function (included) {
		var intafaces = _.rest(arguments),
			...
			j,
			k,
			comp_intaface = Booze.Intaface('Composite', ['add', 'remove', 'get', 'find']),
			leaf = {
				add: noOp,
				remove: noOp,
				get: noOp,
			...
			comp_add = function (comp) {
				...
				Booze.Intaface.ensures.apply(Booze.Intaface, intafaces);
